...
At start up, the robot computer will launch the manipulator's controller using the `clearpath-manipulators.service`. Once the arm is on and initialized through the teach pendant, start the `external_control` program, which will begin communication with the ROS driver.
...
In this mode, you will be unable to interface with the robot directly from the tablet. Instead, all of these commands can be sent directly through the external device.

#### Parameter: Device IP
The `robot_ip` must be set to match the networking settings of the manipulator's control box. By default, we use the `192.168.131.40` address.

#### Parameter: UR Type
The `ur_type` parameter can be modified to change the type of the arm. By default, it is set to `ur5e`. But, it is critical that it is changed to match the real manipulator type. The following types are supported: `ur3`, `ur3e`, `ur5`, `ur5e`, `ur10`, `ur10e`, `ur16e`, `ur20`, `ur30`.
